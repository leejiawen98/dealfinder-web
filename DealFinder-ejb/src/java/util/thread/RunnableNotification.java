...
public class RunnableNotification implements Runnable
{
    ...
    public void run()
    {
       try
       {
           while(true)
           {
               ...
               if(asyncResult != null && asyncResult.isDone()) 
               {
                   ...
                   if(result)
                   {
                       System.out.println("[SERVER] Verification email sent successfully!\n");
                   }
                   else
                   {
                       System.out.println("[SERVER] Verification email was NOT actually sent!\n");
                   }
       ...
       {
           System.out.println("[SERVER] Error actually sending email: " + ex.getMessage() + "!\n");
       }
    }
