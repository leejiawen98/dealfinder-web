...
public class CreditCardResource {
    ...
    public Response createNewCreditCardEntityForCustomer(CreateCreditCardReq createCreditCardReq) {
        if (createCreditCardReq != null) {
            try { 
                ...
                return Response.status(Response.Status.OK).entity(creditCard.getCcId()).build();
    ...
    public Response retrieveCreditCardByCreditCardId(@PathParam("creditCardId") long creditCardId)
    {
        try
        {
            ...
            creditCard.getCustomer().getDeals().clear();
            creditCard.getCustomer().getFavourites().clear();
            creditCard.getCustomer().getRedemptions().clear();
            creditCard.getCustomer().getReviews().clear();
            creditCard.getCustomer().getSaleTransactions();
...
//            creditCard.getCustomer().getSaleTransactions();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    public Response updateCreditCard(CreditCard creditCard)
    {
        if(creditCard != null)
        {
            try
            {                
                System.out.println("********** CustomerResource.updateCreditCard(): CreditCard " + creditCard.getCcNum() + " is being updated.");
                
                creditCardSessionBeanLocal.updateCreditCard(creditCard);
                
                return Response.status(Response.Status.OK).build();
            }
            catch(UpdateCreditCardException ex)
            {
        ...
        {
            return Response.status(Response.Status.BAD_REQUEST).entity("Invalid update credit card request").build();
        }
    }
